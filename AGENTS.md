...
| **Local** | Virtuoso on your own machine | Set `VB_REMOTE_HOST=localhost`, run `virtuoso-bridge start` |

## Prerequisites

1. **SSH**: `ssh my-server` must work without a password prompt.
2. **Virtuoso** (for SKILL execution): a Virtuoso installation/session on the remote (or local) machine; `virtuoso-bridge start` is the normal bootstrap path.
...
`start` is the expected one-command bootstrap: it prepares the per-user
bridge files, starts/reuses the transport, and should bring the RAMIC
daemon online without asking the user to paste SKILL into CIW.
...
If `status` still reports `[daemon] NO RESPONSE`, treat the printed
`load(...)` path as a troubleshooting fallback, not as the normal setup
flow:
...
3. **Check Virtuoso** — `ssh <VB_REMOTE_HOST> "pgrep -f virtuoso"` and/or verify `which virtuoso` after sourcing the Cadence cshrc.
   - If no process exists, prefer the bridge's automatic/headless bootstrap path before asking the user to open Virtuoso.

4. **Start bridge** — `virtuoso-bridge start`
   - If `[daemon] NO RESPONSE`: use the printed `load("...")` only as a fallback/debug hint; do not present manual CIW loading as the standard path.
...
bootstraps the local bridge.  If daemon startup fails, the printed
`load(...)` line is a fallback/debug hint, not the standard flow.
...
|---|---|---|
| **Virtuoso daemon** | Execute SKILL expressions in Virtuoso | `virtuoso-bridge start` bootstrap; manual `load("...virtuoso_setup.il")` is only a fallback/debug path |
...
[daemon]  OK               ← Virtuoso daemon connected (or NO RESPONSE if bootstrap failed)
...
virtuoso-bridge start           # start SSH tunnel + bootstrap daemon
